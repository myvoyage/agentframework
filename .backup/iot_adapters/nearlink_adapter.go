...
func NewNearLinkAdapter() *NearLinkAdapter {
	return &NearLinkAdapter{
		BaseAdapter: iot.NewBaseAdapter(iot.ProtocolNearLink, "1.0.0"),
...
func (a *NearLinkAdapter) Initialize(ctx context.Context, config iot.ProtocolConfig) error {
...
func (a *NearLinkAdapter) DiscoverDevices(ctx context.Context, timeout time.Duration) ([]*iot.DeviceInfo, error) {
	if !a.IsRunning() {
...
func (a *NearLinkAdapter) StartPairing(ctx context.Context, timeout time.Duration) (*iot.PairingResult, error) {
	if !a.IsRunning() {
		return nil, iot.ErrAdapterNotRunning
	}
...
func (a *NearLinkAdapter) GetDevice(ctx context.Context, deviceID string) (iot.IoTDevice, error) {
	a.mutex.RLock()
...
func (a *NearLinkAdapter) ListDevices(ctx context.Context) ([]iot.IoTDevice, error) {
	a.mutex.RLock()
	defer a.mutex.RUnlock()
...
func (a *NearLinkAdapter) GetNetworkInfo(ctx context.Context) (*iot.NetworkInfo, error) {
	if !a.IsRunning() {
		return nil, iot.ErrAdapterNotRunning
	}
...
func (a *NearLinkAdapter) SetEventHandler(handler iot.ProtocolEventHandler) {
	a.eventHandler = handler
}

// PublishEvent publishes an event to the event handler.
func (a *NearLinkAdapter) PublishEvent(event iot.ProtocolEvent) {
...
func (a *NearLinkAdapter) createNearLinkDevice(info *iot.DeviceInfo) *NearLinkDevice {
...
func parseNearLinkDeviceAnnouncement(data []byte, addr *net.UDPAddr) (*iot.DeviceInfo, error) {
	...
	return &iot.DeviceInfo{
		...
		LastSeen:    time.Now(),
		Capabilities: []iot.DeviceCapability{iot.CapabilitySensor},
	}, nil
}
