...
func NewZWaveAdapter() *ZWaveAdapter {
	return &ZWaveAdapter{
		BaseAdapter: NewBaseAdapter(ProtocolZWave, "1.0.0"),
		devices:     make(map[uint8]*ZWaveDevice),
	}
}

// Initialize initializes the Z-Wave adapter with the given configuration.
func (a *ZWaveAdapter) Initialize(ctx context.Context, config ProtocolConfig) error {
...
func (a *ZWaveAdapter) DiscoverDevices(ctx context.Context, timeout time.Duration) ([]*DeviceInfo, error) {
	if !a.IsRunning() {
...
func (a *ZWaveAdapter) StartPairing(ctx context.Context, timeout time.Duration) (*PairingResult, error) {
	if !a.IsRunning() {
		return nil, ErrAdapterNotRunning
	}
...
func (a *ZWaveAdapter) GetDevice(ctx context.Context, deviceID string) (IoTDevice, error) {
	a.mutex.RLock()
...
func (a *ZWaveAdapter) ListDevices(ctx context.Context) ([]IoTDevice, error) {
	a.mutex.RLock()
	defer a.mutex.RUnlock()
...
func (a *ZWaveAdapter) GetNetworkInfo(ctx context.Context) (*NetworkInfo, error) {
	if !a.IsRunning() {
		return nil, ErrAdapterNotRunning
	}
...
func (a *ZWaveAdapter) SetEventHandler(handler ProtocolEventHandler) {
	a.eventHandler = handler
}

// PublishEvent publishes an event to the event handler.
func (a *ZWaveAdapter) PublishEvent(event ProtocolEvent) {
...
func (a *ZWaveAdapter) handleZWaveEvent(message map[string]interface{}) {
	...
	// Publish event
	a.PublishEvent(ProtocolEvent{
		Type:      EventType("zwave_" + eventType),
		Protocol:  ProtocolZWave,
		DeviceID:  deviceID,
		Timestamp: time.Now(),
		Data:      message,
...
func (a *ZWaveAdapter) waitForInclusion(ctx context.Context, timeout time.Duration) (*PairingResult, error) {
...
func (a *ZWaveAdapter) createZWaveDevice(info *DeviceInfo) *ZWaveDevice {
	return &ZWaveDevice{
...
func getZWaveDeviceType(node map[string]interface{}) DeviceType {
...
func getZWaveCapabilities(node map[string]interface{}) []DeviceCapability {
	...
	if isActuator(deviceClass) {
		capabilities = append(capabilities, CapabilityOnOff)
	}
...
// GetJSClient returns the underlying Z-Wave JS client.
func (a *ZWaveAdapter) GetJSClient() *ZWaveJSClient {
	return a.jsClient
}
