...
func NewThreadAdapter() *ThreadAdapter {
	return &ThreadAdapter{
		BaseAdapter: NewBaseAdapter(ProtocolThread, "1.0.0"),
		devices:     make(map[string]*ThreadDevice),
	}
}

// Initialize initializes the Thread adapter with the given configuration.
func (a *ThreadAdapter) Initialize(ctx context.Context, config ProtocolConfig) error {
...
func (a *ThreadAdapter) DiscoverDevices(ctx context.Context, timeout time.Duration) ([]*DeviceInfo, error) {
	if !a.IsRunning() {
	...
	devices := make([]*DeviceInfo, 0, len(discovered))
...
func (a *ThreadAdapter) StartPairing(ctx context.Context, timeout time.Duration) (*PairingResult, error) {
	if !a.IsRunning() {
		return nil, ErrAdapterNotRunning
	}
...
func (a *ThreadAdapter) GetDevice(ctx context.Context, deviceID string) (IoTDevice, error) {
...
func (a *ThreadAdapter) ListDevices(ctx context.Context) ([]IoTDevice, error) {
...
func (a *ThreadAdapter) GetNetworkInfo(ctx context.Context) (*NetworkInfo, error) {
...
func (a *ThreadAdapter) SetEventHandler(handler ProtocolEventHandler) {
	a.eventHandler = handler
}

// PublishEvent publishes an event to the event handler.
func (a *ThreadAdapter) PublishEvent(event ProtocolEvent) {
...
func (a *ThreadAdapter) waitForJoiner(ctx context.Context, timeout time.Duration) (*DeviceInfo, error) {
...
func (br *ThreadBorderRouter) DiscoverDevices(ctx context.Context, timeout time.Duration) ([]*DeviceInfo, error) {
...
// createThreadDevice creates a ThreadDevice from DeviceInfo.
func (a *ThreadAdapter) createThreadDevice(info *DeviceInfo) *ThreadDevice {
	return NewThreadDevice(info, a)
}
