...
func (d *ZigbeeDevice) ID() string {
	return d.deviceID
}

// Name returns the device friendly name.
func (d *ZigbeeDevice) Name() string {
	return d.FriendlyName
}

// Type returns the device type.
func (d *ZigbeeDevice) Type() DeviceType {
	return d.BaseDevice.Type()
}

// Connect connects to the device (virtual connection for Zigbee).
func (d *ZigbeeDevice) Connect(ctx context.Context) error {
	d.setConnected(true)
	d.emitEvent(EventDeviceStatusChanged, map[string]interface{}{
		"status": "connected",
	}, nil)
	return nil
}
...
func (d *ZigbeeDevice) Read(ctx context.Context, attribute string) (interface{}, error) {
	...
	topic := d.adapter.mqttClient.GetDeviceGetTopic(d.deviceID)
...
func (d *ZigbeeDevice) Write(ctx context.Context, attribute string, value interface{}) error {
	...
	// Send to device
	if err := d.adapter.mqttClient.SetDeviceState(ctx, d.deviceID, state); err != nil {
	...
	// Emit event
	d.emitEvent(EventDataReceived, map[string]interface{}{
		"attribute": attribute,
		"value":     value,
	}, nil)
...
func (d *ZigbeeDevice) Subscribe(ctx context.Context, events []string, handler DeviceEventHandler) error {
...
func (d *ZigbeeDevice) emitEvent(eventType EventType, data map[string]interface{}, err error) {
	// Call BaseDevice's emitEvent method
	d.BaseDevice.emitEvent(context.Background(), eventType, data, err)
}

// EmitEvent is the public method to emit events.
func (d *ZigbeeDevice) EmitEvent(ctx context.Context, event DeviceEvent) {
	// This will be called by the adapter when events occur
	d.BaseDevice.emitEvent(ctx, event.Type, event.Data, event.Error)
}

// determineDeviceType determines device type from Zigbee device type.
func determineDeviceType(zigbeeType string) DeviceType {
	switch zigbeeType {
	case "EndDevice":
...
func determineCapabilities(deviceType string) []DeviceCapability {
	...
	case "Router", "EndDevice":
		...
		capabilities = append(capabilities, CapabilityLevelControl)
		...
		if isSensorDevice(deviceType) {
			capabilities = append(capabilities, CapabilitySensor)
		}
	}
