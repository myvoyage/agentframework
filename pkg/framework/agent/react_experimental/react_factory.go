...
type DefaultReActAgentFactory struct {
	BaseAgentFactory
	// LLMFactory LLM工厂
	LLMFactory func() (Model, error)
	// ToolRegistry 工具注册表
	ToolRegistry map[string]tool.BaseTool
	// MemoryManager 内存管理器
	MemoryManager memory.Manager
	// configLoader 配置加载器
	// TODO: Implement proper config loader
...
func WithModelClient(client Model) AgentOption {
	return func(o *agentCreationOptions) {
...
// 【必须】提供构造函数确保必要依赖
func NewDefaultReActAgentFactory(logger *zap.Logger, llmFactory func() (Model, error), toolRegistry map[string]tool.BaseTool, memoryManager memory.Manager) *DefaultReActAgentFactory {
	if logger == nil {
		logger = zap.NewNop()
	}
...
// 【推荐】根据配置创建合适的LLM客户端
func (draf *DefaultReActAgentFactory) createModelClient(ctx context.Context, cfg *ReActConfig, options *agentCreationOptions) (Model, error) {
	// 如果提供了自定义客户端，使用自定义的
	...
	// 使用 LLMFactory 创建模型客户端
	if draf.LLMFactory != nil {
		client, err := draf.LLMFactory()
		if err != nil {
			return nil, errors.WrapError(err, "failed to create model client", map[string]interface{}{
	...
	return nil, errors.NewValidationError("no model client available", nil)
}

// createPlanner 创建规划器
// 【推荐】根据配置创建合适的规划器
func (draf *DefaultReActAgentFactory) createPlanner(ctx context.Context, cfg *ReActConfig, modelClient Model, logger *zap.Logger) (PlanGenerator, error) {
	// 创建规划器配置
