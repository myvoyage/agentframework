...
// 【必须】提供构造函数确保必要配置
func NewToolActionExecutor(logger *zap.Logger, config *ReActConfig, toolRegistry registry.ToolRegistry) *ToolActionExecutor {
...
func (tae *ToolActionExecutor) Execute(ctx context.Context, action *Action, state *ReActState) (*Observation, error) {
	...
	case <-ctx.Done():
		return nil, errors.NewContextError("execution cancelled due to context cancellation", ctx.Err(), nil)
	case <-time.After(tae.ExecutionTimeout):
		return nil, errors.NewTimeoutError("execution timeout waiting for semaphore", tae.ExecutionTimeout, nil)
	}
	...
	tae.logger.Info("tool action executed successfully",
		zap.String("action_id", action.ID),
		zap.String("tool_name", action.Name),
		zap.Duration("execution_time", time.Since(time.Now().Add(-time.Second)), // 简化处理
		)
...
// findTool 查找指定的工具
// 【推荐】从工具注册表中查找工具
func (tae *ToolActionExecutor) findTool(ctx context.Context, toolName string) (agent.Tool, error) {
...
// 【推荐】将动作参数转换为工具可接受的格式
func (tae *ToolActionExecutor) prepareToolParameters(ctx context.Context, tool agent.Tool, actionParams map[string]interface{}) (map[string]interface{}, error) {
	if actionParams == nil {
...
// 【推荐】提取工具的必需参数信息
func (tae *ToolActionExecutor) getRequiredParameters(tool agent.Tool) map[string]bool {
...
func (sae *SearchActionExecutor) performSearch(ctx context.Context, query string, params map[string]interface{}) ([]SearchResult, error) {
	...
	for _, provider := range sae.SearchProviders {
		...
		select {
		case <-ctx.Done():
			return nil, errors.NewContextError("search cancelled", ctx.Err(), nil)
		default:
...
// ActionExecutorManager 动作执行器管理器
// 【必须】管理多个动作执行器的执行
type ActionExecutorManager struct {
	// executors 执行器集合
...
// 【必须】提供构造函数确保必要初始化
func NewActionExecutorManager(logger *zap.Logger) *ActionExecutorManager {
	if logger == nil {
		logger = zap.NewNop()
	}

	return &ActionExecutorManager{
